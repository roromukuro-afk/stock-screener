...
def build_and_save_candidates(market: str = "JP", max_symbols: int = 200,
    ...
    return {
        ...
        # 材料連動診断
        "catalyst_lookup_symbols": r.get("catalyst_lookup_symbols"),
        "catalyst_categories_tracked": r.get("catalyst_categories_tracked"),
        "catalyst_boosted_candidates": r.get("catalyst_boosted_candidates"),
...
def build_candidates(market: str = "JP", max_symbols: int = 200,
    ...
    if catalyst_score:
        ...
        cat_keys = list({k for k in catalyst_score.keys() if k})
    ...
    return {
        ...
        # ★ 材料カバレッジ診断
        "catalyst_lookup_symbols": len(catalyst_score),
        "catalyst_categories_tracked": list(set(catalyst_category_by_sym.values()))[:10],
        "catalyst_boosted_candidates": sum(1 for c in candidates if (c.get("catalyst_boost") or 0) > 0),
    }
